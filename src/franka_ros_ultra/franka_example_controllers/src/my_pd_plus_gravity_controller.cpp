...
#include <franka_example_controllers/my_pd_plus_gravity_controller.h>
...
bool MyPdPlusGravityController::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 20, &MyPdPlusGravityController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("MyPdPlusGravityController: Could not read parameter arm_id");
    return false;
  }
  std::vector<std::string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "MyPdPlusGravityController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MyPdPlusGravityController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "MyPdPlusGravityController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MyPdPlusGravityController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "MyPdPlusGravityController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MyPdPlusGravityController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "MyPdPlusGravityController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&MyPdPlusGravityController::complianceParamCallback, this, _1, _2));
...
void MyPdPlusGravityController::starting(const ros::Time& /*time*/) {
...
void MyPdPlusGravityController::update(const ros::Time& time,
...
void MyPdPlusGravityController::stopping(const ros::Time &time){
  ROS_INFO("Stopping Controller and Saving data......");
  std::string path("/home/yan/yxj/ws_ultrasound/src/data/0605/");
...
Eigen::Matrix<double, 7, 1> MyPdPlusGravityController::saturateTorqueRate(
...
void MyPdPlusGravityController::complianceParamCallback(
...
void MyPdPlusGravityController::equilibriumPoseCallback(
...
//yxj 0603
void MyPdPlusGravityController::saveData(std::vector<Eigen::Matrix<double,6,1>> &Data, std::string filePath){
...
void MyPdPlusGravityController::saveData_time(std::vector<double> &Data, std::string filePath){
  //format: 4 bytes vector number + 4 bytes totalsize + data
	std::ofstream ofile(filePath.c_str(), std::ios::binary);
	if(ofile.is_open()==false){
		std::cout<<"Open file fail!"<<std::endl;
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::MyPdPlusGravityController,
