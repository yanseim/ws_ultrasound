...
#include <franka_example_controllers/my_inverse_dynamic_controller.h>
...
bool MyInverseDynamicController::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 20, &MyInverseDynamicController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("MyInverseDynamicController: Could not read parameter arm_id");
    return false;
  }
  std::vector<std::string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "MyInverseDynamicController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MyInverseDynamicController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "MyInverseDynamicController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MyInverseDynamicController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "MyInverseDynamicController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "MyInverseDynamicController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "MyInverseDynamicController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&MyInverseDynamicController::complianceParamCallback, this, _1, _2));
...
void MyInverseDynamicController::starting(const ros::Time& /*time*/) {
  ...
  orientation_d_matrix_ = initial_transform.linear();
...
void MyInverseDynamicController::update(const ros::Time& time,
  ...
  Eigen::Vector3d position(transform.translation());
  Eigen::Quaterniond orientation(transform.linear());
  Eigen::Matrix<double, 3, 3> orientation_matrix(transform.linear());
  ...
  jacobian_analytic.block<3,7>(3,0) = 0.5 * T * jacobian_analytic.block<3,7>(3,0);
  ...
  // yxj compute error through matrix
  ...
  // tau_task << M * jacobian_analytic_pinv * (-K_d * error - D_d * error_dot - jacobian_analytic_dot *dq);
  tau_task << M * jacobian_pinv * (-K_d * error - D_d * error_dot - jacobian_dot *dq);
  ...
  //cartesian inverse dynamic control
  ...
  if (yxj_counter%1000==0){
    ...
    // std::cout<< "-K_d * error - D_d * error_dot - jacobian_analytic_dot *dq"<<std::endl<<-K_d * error - D_d * error_dot - jacobian_analytic_dot *dq<<std::endl;
    ...
    // std::cout<<"orientation.coeffs()"<<std::endl<<orientation.coeffs()<<std::endl;
    // std::cout<<"T"<<std::endl<<0.5*T.transpose()<<std::endl;
    // std::cout<<"J"<<std::endl<<jacobian<<std::endl;
    // std::cout<<"JA"<<std::endl<<jacobian_analytic<<std::endl;
    // std::cout<<"J_dot"<<std::endl<<jacobian_dot<<std::endl;

    // std::cout<<"error_o"<<std::endl<<error_o<<std::endl;

    // std::cout<<"orientation_matrix"<<std::endl<<orientation_matrix<<std::endl;
  }
...
void MyInverseDynamicController::stopping(const ros::Time &time){
  ROS_INFO("Stopping Controller and Saving data......");
  std::string path("/home/yan/yxj/ws_ultrasound/src/data/0617/");
...
Eigen::Matrix<double, 7, 1> MyInverseDynamicController::saturateTorqueRate(
...
void MyInverseDynamicController::complianceParamCallback(
...
void MyInverseDynamicController::equilibriumPoseCallback(
...
//yxj 0603
void MyInverseDynamicController::saveData(std::vector<Eigen::Matrix<double,6,1>> &Data, std::string filePath){
...
void MyInverseDynamicController::saveData_time(std::vector<double> &Data, std::string filePath){
  //format: 4 bytes vector number + 4 bytes totalsize + data
	std::ofstream ofile(filePath.c_str(), std::ios::binary);
	if(ofile.is_open()==false){
		std::cout<<"Open file fail!"<<std::endl;
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::MyInverseDynamicController,
